...
public class Part2Main {
	...
	public static <E> void main(String args[]) {
		...
		for (int size = isize; size <= fsize; size += istep) {
			...
			for (StrategiesTimeCollection<E> strategy : resultsPerStrategy)
				strategy.resetSum(); // set to 0 an internal instance variable
										// used to accumulate sum of times
			...
			for (int r = 0; r < rep; r++) {
				...
				for (StrategiesTimeCollection<E> strategy : resultsPerStrategy) {
					long startTime = System.nanoTime(); // Measure system�s
														// clock time before.

					System.out.println("run trial " + count++);

					strategy.runTrial((E[][][]) dataset); // Run the strategy
															// using the data in
															// dataset.
					long endTime = System.nanoTime(); // Measure system�s clock
														// time after.

					int estimatedTime = (int) (endTime - startTime); // The
																		// estimated
																		// time.
					// Accumulate the estimated time (add it) to sum of times
					// that the current
					// strategy has
					// so far accumulated on the previous trials for datasets of
					// the current size.
	...
	public static void allStrategyResults(StrategiesTimeCollection<Object>[] results)
		...
		for (int x = 0; x < results[0].getList().size(); x++) { // for each size
			for (int i = 0; i < results.length; i++) { // for each strategy
				if (i == 0) {
					writer.write(Integer.toString((results[i].getList().get(x).getKey()))); // write
																							// the
																							// size
																							// appropriate
																							// to
				...
				writer.write("\t" + Float.toString(results[i].getList().get(x).getValue())); // Add
																								// the
																								// time
																								// in
																								// the
			...
			writer.write("\r\n");// Line break for the next set of times per
									// size
	...
	public static Object[][][] generateData(int n, int m, int size) {
		Object[][][] dataSet = new Integer[m][n][];
